...
class Slider : public HitObject {
   public:
    ...
    std::vector<std::vector<bool>> renderedLocations = {
        std::vector<std::vector<bool>>(945, std::vector<bool>(785, false))};
   ...
   private:
    ...
    float minX = static_cast<float>(INT_MAX), minY = static_cast<float>(INT_MAX), maxX = INT_MIN,
          maxY = INT_MIN;
};
