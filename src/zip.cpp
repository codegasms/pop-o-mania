...
static char *zip_name_normalize(char *name, char *const nname, size_t len) {
    ...
    // skip trailing '/'
    while (ISSLASH(*name)) name++;

    for (; offn < len; offn++) {
        if (ISSLASH(name[offn])) {
            if (ncpy > 0 && strcmp(&nname[offnn], ".\0") && strcmp(&nname[offnn], "..\0")) {
                offnn += ncpy;
...
static int zip_archive_extract(mz_zip_archive *zip_archive, const char *dir,
    ...
    for (i = 0; i < n; ++i) {
        ...
        if ((((info.m_version_made_by >> 8) == 3) ||
             ((info.m_version_made_by >> 8) == 19))    // if zip is produced on Unix or macOS (3 and
                                                       // 19 from section 4.4.2.2 of zip standard)
            && info.m_external_attr & (0x20 << 24)) {  // and has sym link attribute (0x80 is file,
                                                       // 0x40 is directory)
#if defined(_WIN32) || defined(__WIN32__) || defined(_MSC_VER) || defined(__MINGW32__)
#else
            if (info.m_uncomp_size > MAX_PATH ||
                !mz_zip_reader_extract_to_mem_no_alloc(zip_archive, i, symlink_to, MAX_PATH, 0,
                                                       NULL, 0)) {
                err = ZIP_EMEMNOALLOC;
                goto out;
            }
...
static ssize_t zip_entry_mark(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
                              const ssize_t n, char *const entries[], const size_t len) {
    ssize_t i = 0;
    ssize_t err = 0;
...
static int zip_index_update(struct zip_entry_mark_t *entry_mark, ssize_t last_index,
                            ssize_t nxt_index) {
    ssize_t j;
...
static int zip_entry_finalize(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
                              const ssize_t n) {
    ssize_t i = 0;
...
static ssize_t zip_entry_set(struct zip_t *zip, struct zip_entry_mark_t *entry_mark, ssize_t n,
...
static ssize_t zip_file_move(MZ_FILE *m_pFile, const mz_uint64 to, const mz_uint64 from,
                             const size_t length, mz_uint8 *move_buf, const size_t capacity_size) {
...
static ssize_t zip_files_move(MZ_FILE *m_pFile, mz_uint64 writen_num, mz_uint64 read_num,
                              size_t length) {
    ssize_t n = 0;
    ...
    while ((mz_int64)length > 0) {
        ...
        n = zip_file_move(m_pFile, writen_num, read_num, move_count, move_buf, page_size);
        if (n < 0) {
            moved_length = n;
            goto cleanup;
        }
...
static int zip_central_dir_delete(mz_zip_internal_state *pState, int *deleted_entry_index_array,
                                  int entry_num) {
    int i = 0;
    int begin = 0;
    int end = 0;
...
static ssize_t zip_entries_delete_mark(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
                                       int entry_num) {
...
static int _zip_entry_open(struct zip_t *zip, const char *entryname, int case_sensitive) {
    ...
    */
    ...
    if (!mz_zip_writer_write_zeros(pzip, zip->entry.offset, num_alignment_padding_bytes)) {
    ...
    if (pzip->m_pWrite(pzip->m_pIO_opaque, zip->entry.offset, zip->entry.name, entrylen) !=
        entrylen) {
    ...
    if (pzip->m_pWrite(pzip->m_pIO_opaque, zip->entry.offset, extra_data, extra_size) !=
        extra_size) {
    ...
    if (level) {
        ...
        if (tdefl_init(&(zip->entry.comp), mz_zip_writer_add_put_buf_callback, &(zip->entry.state),
...
int zip_entry_extract(struct zip_t *zip,
                      size_t (*on_extract)(void *arg, uint64_t offset, const void *buf,
                                           size_t bufsize),
                      void *arg) {
    mz_zip_archive *pzip = NULL;
    mz_uint idx;
