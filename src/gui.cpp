...
TestSlider::TestSlider(Vector2 position, Vector2 size, Color color, Color altcolor, Color outcolor,
                       Color switchcolor)
    : GuiElement(position, size),
      color(color),
      altcolor(altcolor),
...
void TestSlider::render() {
    ...
    Rectangle Left = GetRaylibOriginR(
        {position.x - (size.x * (1 - percentage)) / 2, position.y, size.x * percentage, size.y});
    Rectangle Right = GetRaylibOriginR({position.x + (size.x * (percentage)) / 2, position.y,
                                        size.x * (1.0f - percentage), size.y});
    Rectangle Dingus = GetRaylibOriginR(
        {position.x - (size.x / 2) + (size.x * (percentage)), position.y, size.y / 2, size.y});
    if (Dingus.x < position.x - (size.x / 2)) Dingus.x = position.x - (size.x / 2);
...
void TestSlider::update() {
    ...
    if (this->focused && Global.Wheel > 0) {
        location += 1;
        if (location > 100.0) location = 100;
    ...
    if (Dingus.x > position.x + (size.x / 2) - size.y / 2)
        Dingus.x = position.x + (size.x / 2) - size.y / 2;
    if (Global.Key1P && CheckCollisionPointRec(Global.MousePosition, Dingus)) active = true;
    if (!Global.Key1D) active = false;
