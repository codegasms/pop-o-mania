...
void SleepInUs(uint32 us) {
    ::LARGE_INTEGER ft;
    ft.QuadPart = -static_cast<int64>(us) * 10;  // 100 nanoseconds
...
static inline uint64 PerfFrequency() {
    static uint64 freq = GetPerfFrequency();
    return freq;
}
...
uint64 NowInUs() {
    return static_cast<uint64>(static_cast<double>(PerfCounter()) / PerfFrequency() * 1000000);
}
#endif
