...
GameFile Parser::parseMetadata(std::string filename) {
    ...
    if (ifs.is_open()) {
        while (std::getline(ifs, line)) {
            if (line[line.size() - 1] == 13) line.pop_back();
            if (line[0] == '[' and line[line.size() - 1] == ']') {
                ...
                if (header == "Metadata") {
                    ...
                    while (std::getline(ifs, subLine)) {
                        ...
                        if (subLine[subLine.size() - 1] == 13) subLine.pop_back();
                        if (subLine.size() == 0) break;
                        if (subLine[0] == '/' and subLine[1] == '/') continue;
                        std::pair<std::string, std::string> keyValue =
                            parseKeyValue(subLine, false, false);
            ...
            if (line[0] == '[' and line[line.size() - 1] == ']') {
                ...
                if (header == "General") {
                    ...
                    while (std::getline(ifs, subLine)) {
                        ...
                        std::pair<std::string, std::string> keyValue =
                            parseKeyValue(subLine, false, true);
                    ...
                    while (std::getline(ifs, subLine)) {
                        ...
                        if (typeInBinary[0] == '1') {
                            ...
                            tempHitObject.skipComboColours = int(typeInBinary[4] == '1') +
                                                             int(typeInBinary[5] == '1') * 2 +
                                                             int(typeInBinary[6] == '1') * 4;
                            ...
                            if (tempVectorCurvePoints.size() - 1 >= 3 &&
                                tempHitObject.curveType == 'P') {
                                tempHitObject.curveType = 'B';
                            }

                            for (size_t i = 1; i < tempVectorCurvePoints.size(); i++) {
                                std::vector<std::string> tempVectorCurvePointCords;
                                tempVectorCurvePointCords =
                                    parseSeperatedLists(tempVectorCurvePoints[i], ':');
                                tempHitObject.curvePoints.push_back(
                                    std::make_pair(std::stoi(tempVectorCurvePointCords[0]) + 64,
                                                   std::stoi(tempVectorCurvePointCords[1]) + 48));
                            }
                            ...
                            if (tempVector.size() > 8) {
                                ...
                                for (size_t i = 0; i < tempVectorEdgeSounds.size(); i++)
                                    tempHitObject.edgeSounds.push_back(
                                        std::stoi(tempVectorEdgeSounds[i]));
                            } else {
                            ...
                            if (tempVector.size() > 9) {
                                ...
                                for (size_t i = 0; i < tempVectorEdgeSets.size(); i++) {
                                    std::vector<std::string> tempVectorEdgeSetsCords;
                                    tempVectorEdgeSetsCords =
                                        parseSeperatedLists(tempVectorEdgeSets[i], ':');
                                    tempHitObject.edgeSets.push_back(
                                        std::make_pair(std::stoi(tempVectorEdgeSetsCords[0]),
                                                       std::stoi(tempVectorEdgeSetsCords[1])));
                                }
                            } else {
                                for (int i = 0; i < tempHitObject.slides; i++) {
                                    std::vector<std::string> tempVectorEdgeSetsCords;
...
std::pair<std::string, std::string> Parser::parseKeyValue(std::string line, bool hasSpaceBefore,
                                                          bool hasSpaceAfter) {
    size_t spliceLocation;
    for (spliceLocation = 0; spliceLocation < line.size(); spliceLocation++)
        if (line[spliceLocation] == ':') break;
    return make_pair(line.substr(0, spliceLocation - hasSpaceBefore),
                     line.substr(spliceLocation + hasSpaceAfter + 1,
                                 line.size() - 1 - spliceLocation - hasSpaceAfter));
}
...
std::vector<std::string> Parser::parseSeperatedLists(std::string list, char seperator) {
    ...
    for (size_t i = 0; i < list.size(); i++) {
        if (list[i] == seperator or i == list.size() - 1) {
            ans.push_back(list.substr(lastCommaLocation + 1,
                                      i - lastCommaLocation - 1 + int(i == list.size() - 1)));
            lastCommaLocation = i;
        }
    }
    return ans;
